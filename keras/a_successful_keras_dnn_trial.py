...
def build_model(columns):
    ...
    model.compile(optimizer='adam', loss=keras.losses.binary_crossentropy)     # "rmsprop  adam"    "binary_crossentropy mse"
    return model
...
def fit_and_predict_model(train_feature, train_label, validate_feature, validate_label, test_feature, store_result=False):
    ...
    early_stopping = EarlyStopping(monitor='loss', patience=20)  # val_loss
    model.fit(train_feature, train_label, batch_size=1000, epochs=200, verbose=2, validation_data=[validate_feature, validate_label], callbacks=[early_stopping])    # , validation_data=[te_x, te_y] #
